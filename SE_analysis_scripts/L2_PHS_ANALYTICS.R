...
phs<-read_tsv(paste0(directory, "Data/Raw/raw_phs.txt"), col_types = cols(.default = "c"))
...
rm("phs_program_start", "phs_phs", "phs_met_goal", "phs_bmi_at_risk", "phs_inactive", "phs_smoker", 
   ...
   'phs_address_2', 'phs_address_state', 'phs_address_city', 'phs_IH_id')
...
phs <- phs %>% mutate('assessment' = ymd(assessment),
...
original_assessments_tab <- phs %>% distinct(assessment) %>% arrange(assessment) %>% filter(assessment < analysis_date)
